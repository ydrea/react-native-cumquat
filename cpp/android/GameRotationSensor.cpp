  ...
  void run() {
    ...
    while (running.load()) {
      ...
      while (ASensorEventQueue_getEvents(queue, &event, 1) > 0) {
        if (event.type != ASENSOR_TYPE_GAME_ROTATION_VECTOR) continue;
...
bool GameRotationSensor::latest(Quaternion& orientation) const {
  ...
  if (!impl_->hasOrientation) return false;
  orientation = impl_->latestOrientation;
  return true;
}
